<template>
  <div>
    <div>
    <h1 style="text-align:center;">校园网络拓扑可视化</h1>
...
//   return data;
// }
function generateCircularLayout(nodes, links) {
  // 根据层数对节点进行分组
  const groups = {};
  nodes.forEach(node => {
  ...
  // 计算每层节点的位置
  const radius = 150;
  const angleStep = 2 * Math.PI / Object.keys(groups).length;
  let angle = 0;
  const positions = {};
  ...
  // 将节点和连接关系组合成3dforcegraph需要的数据格式
...
function createTopology(nodes, links) {
  const width = 800;
  const height = 600;
  const radius = 100;
  const nodeRadius = 30;
  ...
  const simulation = d3.forceSimulation(nodes)
    ...
    .force("y", d3.forceY().y(d => d.level * radius))
...
export default {
  ...
  methods: {
    ...
    if (buttonValue === 'orient') {
        ...
        // this.A = initializeNodes(this.treeData);
      } else if (buttonValue === 'forceatlas2') {
        const newdata = this.treeData;
        const simulation = d3.forceSimulation(newdata)
          // .force("link", d3.forceLink(newdata.links).id(d => d.id))
          // .force("charge", d3.forceManyBody().strength(-300))
